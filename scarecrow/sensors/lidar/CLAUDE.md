...
- `gazebo.py` — `GazeboLidar`: reads 2D lidar from Gazebo via `gz topic -e -n 1` CLI subprocess. Background thread polling with configurable num_threads for higher scan rate. Auto-discovers lidar topic if not specified.
