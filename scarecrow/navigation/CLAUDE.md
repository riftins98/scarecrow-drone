...
- `mission_recorder.py` — `MissionRecorder` and `RouteRecorder`: everything a mission records for its customer-facing map. Replaced five parallel pieces of mission state (a MapUnit, an events list, a route-sample list, a phase dict, and in-flight sampling tasks) that were previously threaded through the mission by hand.
- `arena.py` — Derives an arena boundary from flight evidence, combining two independent estimates: a box inferred from the stabilised start pose plus its four lidar readings (available immediately, but only as good as one scan from one place), and wall hits projected from route samples as the circuit proceeds. `arena_boundary_from_start()`, `project_route_sample_wall_hit()`, `refine_boundary_from_route_samples()`.
