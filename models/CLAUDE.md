...
- `lidar_2d_v2/` — 2D scanning lidar sensor plugin (`gpu_lidar`, 1440 samples, 360 degrees, `update_rate=30` sim-time). Not to be confused with the single-ray `lidar` sensor inside `holybro_x500/model.sdf` (`samples=1`, 50Hz), which is the downward rangefinder. Simulates an RPLidar A1M8 -- **but the real A1M8 spins at roughly 5.5 Hz**, so the sim delivers several times more scans than the hardware will. Controllers tuned against sim rates will track more loosely on the drone; the rate-dependent constants are fixed, so it degrades rather than misbehaves.
