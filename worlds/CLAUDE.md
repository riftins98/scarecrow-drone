...
- `hangar_lite.sdf` — Lightweight 12m x 8m x 8m hangar shell with checkerboard floor tiles for optical flow, flat ceiling, plain perimeter walls, reduced lighting, one `pigeon_3d` target, and fixed `mono_cam_hd` observer cameras. Layout is aligned so stream launch default spawn (`PX4_GZ_MODEL_POSE=5,-4.5,0,0,0,0`) keeps the same front/right wall distances as `hangar_1` (7.0m front, 3.0m right).
