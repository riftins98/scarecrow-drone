...
- `hangar_circuit_pursiot.py` — Hangar-lite pigeon mission: start 3m from walls, run a 4-leg left-wall circuit with live-only YOLO detection, interrupt into reusable target pursuit, hover at target distance, return/reverse to the interrupted leg start, land with lidar hold, and save a customer-facing mission map.
