...
- `CameraStream.tsx` — Embeds the headless Gazebo MJPEG camera by iframing the launcher's own viewer page (same origin, so no CORS). States: no-url (GUI mode / URL not surfaced yet), standby (sim launching), live. Has live camera-switch plumbing (`setSimCamera`) and remounts the iframe via a nonce to force a fresh load.
