...
- `SpawnPicker.tsx` — Top-down room map (SVG, real meters → SVG, +x = north/up) for choosing the drone's start location. Draws the valid interior (olive, clickable) and the ≥3m wall margin (red hatched, clicks rejected). Click places an olive arrow marker; reports X,Y in meters. Reused pre-connect (initial spawn) and post-connect (re-spawn).
