...
- `SpawnPicker.tsx` — Top-down room map (SVG, real meters → SVG, +x = north/up) for choosing the drone's start location. Draws the valid interior (olive, clickable), the ≥3m wall margin (red hatched), and the two parked-aircraft footprints (red rotated rectangles, from `spawnObstacles`/`spawnObstacleMargin`). Clicks in the margin OR on an aircraft are rejected (mirrors the backend `inObstacle`/`validate_spawn`). Click places an olive arrow marker; reports X,Y in meters. Reused pre-connect (initial spawn) and post-connect (re-spawn).
