...
/**
 * Top-down map of the REAL garage, drawn to scale from the world geometry:
 * the 24x15m room walls, the two parked aircraft (airplane silhouettes), and
 * the landing pads. The drone marker sits at its live world pose (queried from
 * Gazebo via simStatus.dronePose), falling back to the session spawn point when
 * a live pose isn't available. A short trail follows the drone while flying.
 */
export default function Minimap({ simStatus, flightStatus, options }: Props) {
  const connected = !!simStatus?.connected;
  const flying = !!flightStatus?.isFlying;
  const obstacles = options?.spawnObstacles ?? [];
  ...
  const wallTL = worldToSvg(ROOM_X, -ROOM_Y);
  const wallBR = worldToSvg(-ROOM_X, ROOM_Y);
  const drone = worldToSvg(droneX, droneY);

  // Landing pads sit under each aircraft center (RQ-7B footprints in the SDF).
  const pads = obstacles.map((o) => worldToSvg(o.cx, o.cy));
  ...
  return (
    <div className={`minimap ${connected ? 'on' : 'off'}`}>
      ...
      <div className="minimap-scope">
        <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} className="minimap-svg">
          <defs>
          ...
          {/* Parked aircraft (real footprints, airplane silhouettes). */}
          {obstacles.map((o, i) => (
            <path key={i} d={airplanePath(o)}
          ...
          {/* Drone marker at live pose (heading 0 = north/up). */}
          {connected && (
