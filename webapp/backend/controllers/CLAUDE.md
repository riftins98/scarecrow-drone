...
- `sim_controller.py` — `/api/sim/*` (ADD A.1): connect (POST body `{world, headless, spawn?}`), disconnect, status (includes `world`, `headless`, `streamUrl`), camera (live stream swap), **`/api/sim/log/stream`** SSE launcher stdout plus `/api/sim/log` cursor fallback, **`/api/sim/options`** (lists every world in `worlds/` + every flight script in `scripts/flight/` with parsed argparse metadata, plus SDF-derived per-world spawn maps in `worlds[].spawn`/`spawnMaps`), **`/api/sim/reset`** — the panic reset: hard-kills the flight script (`detection_service.kill()`), disarms via PX4 console (`sim_service.disarm_via_console()`), teleports the drone to spawn (`sim_service.reset_drone_pose()`), reapplies launch-time drone values (`sim_service.reset_drone_values_via_console()`), and marks the flight aborted (each step best-effort); and **`/api/sim/spawn`** (POST `{x,y}`) — re-spawn the drone on a running mapped world: validates the >=2m wall margin and static-obstacle clearance, teleports, and updates where reset returns. `/status` returns the current `spawn`.
- `flight_controller.py` — `/api/flights/*` (ADD A.4) + legacy `/api/flight/start|stop|status|log|log/stream`. The legacy routes stay for frontend compatibility until the React side is updated.
